...
func (robot *Robot) SendMessage(msg interface{}) error {
	...
	if err != nil {
		return fmt.Errorf(
			"send dingTalk message failed, error: %v",
			err.Error(),
		)
	}
	...
	if res.StatusCode != 200 {
		return fmt.Errorf(
			"send dingTalk message failed, %s",
			httpError(request, res, result, "http code is not 200"),
		)
	}
	if err != nil {
		return fmt.Errorf(
			"send dingTalk message failed, %s",
			httpError(request, res, result, err.Error()),
		)
	}
	...
	if ret.ErrCode != 0 {
		return fmt.Errorf(
			"send dingTalk message failed, %s",
			httpError(request, res, result, "errcode is not 0"),
		)
	}
...
func (robot *Robot) SendTextMessage(
	content string,
	atMobiles []string,
	isAtAll bool,
) error {
...
func (robot *Robot) SendMarkdownMessage(
	title string,
	text string,
	atMobiles []string,
	isAtAll bool,
...
func (robot *Robot) SendLinkMessage(
	title string,
	text string,
	messageUrl string,
	picUrl string,
